...
static void             RenderMainWindowDecorations(ImGuiWindow* window, const ImRect& title_bar_rect, bool title_bar_is_highlight, int resize_grip_count, const ImU32 resize_grip_col[4], float resize_grip_draw_size);
...
bool ImGui::BeginChildEx(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags)
{
    ...
    parent_window->DrawList->AddRectFilled(ImGui::GetWindowPos(), ImGui::GetWindowPos() + ImVec2(size_arg.x, size_arg.y), ImColor(18, 18, 18, (int)(GetStyle().Alpha * 255.f)), 6, ImDrawCornerFlags_Bot);
    parent_window->DrawList->AddRect(ImGui::GetWindowPos() - ImVec2(0, 32), ImGui::GetWindowPos() + size_arg, ImColor(28, 28, 28, int(255 * g.Style.Alpha)), 6);
    parent_window->DrawList->AddRectFilled(ImGui::GetWindowPos() - ImVec2(0, 32), ImGui::GetWindowPos() + ImVec2(size_arg.x, 0), ImColor(26, 26, 27, (int)(GetStyle().Alpha * 255.f)), 6, ImDrawCornerFlags_Top);
    ...
    parent_window->DrawList->AddImageRounded(ImGui::GetIO().Maintexture, ImGui::GetIO().OriginalWindowPos, ImVec2(ImGui::GetIO().OriginalWindowPos.x + 690, ImGui::GetIO().OriginalWindowPos.y + 470), ImVec2(0, 0)
        , ImVec2(1, 1), ImColor(255, 255, 255, int(255 * g.Style.Alpha)), 6, ImDrawCornerFlags_Top);
    parent_window->DrawList->PopClipRect();
    PushFont(fonts::isis);
    parent_window->DrawList->AddText({ ImGui::GetWindowPos().x + 12,ImGui::GetWindowPos().y - (16 + CalcTextSize(name).y / 2) }, GetColorU32(ImVec4(87 / 255.0f, 88 / 255.0f, 88 / 255.0f, g.Style.Alpha)), name);
    PopFont();
    Spacing(); return ret;
}
    ...
    else
    {
        ...
        if (flags & ImGuiWindowFlags_NoDecoration && !(flags & ImGuiWindowFlags_NoBackground) && !(flags & ImGuiWindowFlags_NoBackground) && (flags & ImGuiWindowFlags_UnsavedDocument))
        {
            ...
            window->DrawList->AddRectFilled(ImVec2(window->Pos.x, window->Pos.y + 30),
                ImVec2(window->Pos.x + window->Size.x, window->Pos.y + window->Size.y), ImColor(16, 16, 16, (int)(GetStyle().Alpha * 155.f)), 6, ImDrawCornerFlags_Bot);
            //Upper bar
            window->DrawList->AddRectFilled(ImVec2(window->Pos),
                ImVec2(window->Pos.x + window->Size.x, window->Pos.y + 28), ImColor(26, 26, 27, (int)(GetStyle().Alpha * 255.f)), 6, ImDrawCornerFlags_Top);
            window->DrawList->AddRectFilledMultiColor(ImVec2(window->Pos.x, window->Pos.y + 28),
                ImVec2(window->Pos.x + window->Size.x, window->Pos.y + 30), ImColor(3, 169, 249, (int)(GetStyle().Alpha * 255.f)), ImColor(43, 116, 255, (int)(GetStyle().Alpha * 255.f)), ImColor(43, 116, 255, (int)(GetStyle().Alpha * 255.f)), ImColor(3, 169, 249, (int)(GetStyle().Alpha * 255.f)));

            //if (flags & ImGuiWindowFlags_KeybindWindow)
            //{
            //    PushFont(GetIO().FontIcons);
            //    window->DrawList->AddText(ImVec2(window->Pos.x + 10, window->Pos.y + 28 / 2 - CalcTextSize("4").y / 2), ImColor(19, 148, 250, (int)(255 * GetStyle().Alpha)), "4");
            //    PopFont();

            //    PushFont(GetIO().Font);
            //    window->DrawList->AddText(ImVec2(window->Pos.x + 40, window->Pos.y + 28 / 2 - CalcTextSize("Keybindings").y / 2), ImColor(250, 250, 250, (int)(255 * GetStyle().Alpha)), "Keybindings");
            //    PopFont();
            //}
            //else
            //{
            //    PushFont(GetIO().FontIcons);
            //    window->DrawList->AddText(ImVec2(window->Pos.x + 10, window->Pos.y + 28 / 2 - CalcTextSize("M").y / 2), ImColor(19, 148, 250, (int)(255 * GetStyle().Alpha)), "M");
            //    PopFont();

            //    PushFont(GetIO().Font);
            //    window->DrawList->AddText(ImVec2(window->Pos.x + 40, window->Pos.y + 28 / 2 - CalcTextSize("Spectating").y / 2), ImColor(250, 250, 250, (int)(255 * GetStyle().Alpha)), "Spectating");
            //    PopFont();
            //}
        ...
        if (flags & ImGuiWindowFlags_Popup && std::string(window->Name).at(0) == '#' && std::string(window->Name).at(1) == '#' && std::string(window->Name).at(1) == 'C')
        {
            window->DrawList->AddRectFilled(window->Pos + ImVec2(0, 0), window->Pos + window->Size, ImColor(ImVec4(22 / 255.f, 22 / 255.f, 22 / 255.f, 0.9f)), 4.f, ImDrawFlags_RoundCornersBottom);
            window->DrawList->AddRect(window->Pos + ImVec2(0, 0), window->Pos + window->Size, ImColor(ImVec4(30 / 255.f, 30 / 255.f, 30 / 255.f, 0.9f)), 4.f, ImDrawFlags_RoundCornersBottom);

        }

        if (flags & ImGuiWindowFlags_Popup && GetID("##picker"))
        {
            window->DrawList->AddRectFilled(window->Pos + ImVec2(0, 0), window->Pos + window->Size, ImColor(ImVec4(22 / 255.f, 22 / 255.f, 22 / 255.f, 0.9f)), 4.f, ImDrawFlags_RoundCornersBottom);
            window->DrawList->AddRect(window->Pos + ImVec2(0, 0), window->Pos + window->Size, ImColor(ImVec4(30 / 255.f, 30 / 255.f, 30 / 255.f, 0.9f)), 4.f, ImDrawFlags_RoundCornersBottom);
            return;
...
void ImGui::RenderMainWindowDecorations(ImGuiWindow* window, const ImRect& title_bar_rect, bool title_bar_is_highlight, int resize_grip_count, const ImU32 resize_grip_col[4], float resize_grip_draw_size)
{
    ImGuiContext& g = *GImGui;
    ImGuiStyle& style = g.Style;
    ImGuiWindowFlags flags = window->Flags;
    ...
    else
    {
        ...
        if (!(flags & ImGuiWindowFlags_NoBackground))
        {
            ...
            if (override_alpha)
                bg_col = (bg_col & ~IM_COL32_A_MASK) | (IM_F32_TO_INT8_SAT(alpha) << IM_COL32_A_SHIFT);
            //Background
            window->DrawList->AddRectFilled(ImVec2(window->Pos.x + 128, window->Pos.y + 40),
                ImVec2(window->Pos.x + window->Size.x, window->Pos.y + window->Size.y - 40), ImColor(16, 16, 16, (int)(GetStyle().Alpha * 255.f)));
            //Upper bar
            window->DrawList->AddRectFilled(ImVec2(window->Pos),
                ImVec2(window->Pos.x + window->Size.x, window->Pos.y + 38), ImColor(26, 26, 27, (int)(GetStyle().Alpha * 255.f)), 6, ImDrawCornerFlags_Top);
            window->DrawList->AddRectFilledMultiColor(ImVec2(window->Pos.x, window->Pos.y + 38),
                ImVec2(window->Pos.x + window->Size.x, window->Pos.y + 40), ImColor(3, 169, 249, (int)(GetStyle().Alpha * 255.f)), ImColor(43, 116, 255, (int)(GetStyle().Alpha * 255.f)), ImColor(43, 116, 255, (int)(GetStyle().Alpha * 255.f)), ImColor(3, 169, 249, (int)(GetStyle().Alpha * 255.f)));

            window->DrawList->PushClipRect(ImVec2(window->Pos),
                ImVec2(window->Pos.x + window->Size.x, window->Pos.y + 38), false);
            window->DrawList->AddImageRounded(ImGui::GetIO().Maintexture, ImGui::GetIO().OriginalWindowPos, ImVec2(ImGui::GetIO().OriginalWindowPos.x + 690, ImGui::GetIO().OriginalWindowPos.y + 470), ImVec2(0, 0)
                , ImVec2(1, 1), ImColor(255, 255, 255, int(255 * g.Style.Alpha)), 6, ImDrawCornerFlags_Top);
            window->DrawList->PopClipRect();


            //Lower bar
            window->DrawList->AddRectFilled(ImVec2(window->Pos.x, window->Pos.y + window->Size.y - 38),
                ImVec2(window->Pos.x + window->Size.x, window->Pos.y + window->Size.y), ImColor(26, 26, 27, (int)(GetStyle().Alpha * 255.f)), 6, ImDrawCornerFlags_Bot);
            window->DrawList->AddRectFilledMultiColor(ImVec2(window->Pos.x, window->Pos.y + window->Size.y - 40),
                ImVec2(window->Pos.x + window->Size.x, window->Pos.y + window->Size.y - 38), ImColor(3, 169, 249, (int)(GetStyle().Alpha * 255.f)), ImColor(43, 116, 255, (int)(GetStyle().Alpha * 255.f)), ImColor(43, 116, 255, (int)(GetStyle().Alpha * 255.f)), ImColor(3, 169, 249, (int)(GetStyle().Alpha * 255.f)));
            window->DrawList->PushClipRect(ImVec2(window->Pos.x, window->Pos.y + window->Size.y - 38), ImVec2(window->Pos.x + window->Size.x, window->Pos.y + window->Size.y), true);
            window->DrawList->AddImageRounded(ImGui::GetIO().Maintexture, ImGui::GetIO().OriginalWindowPos, ImVec2(ImGui::GetIO().OriginalWindowPos.x + 690, ImGui::GetIO().OriginalWindowPos.y + 470), ImVec2(0, 0)
                , ImVec2(1, 1), ImColor(255, 255, 255, int(255 * g.Style.Alpha)), 6, ImDrawCornerFlags_Bot);
            window->DrawList->PopClipRect();
            //Side bar
            window->DrawList->AddRectFilled(ImVec2(window->Pos.x, window->Pos.y + 40),
                ImVec2(window->Pos.x + 128, window->Pos.y + window->Size.y - 40), ImColor(18, 18, 18, (int)(GetStyle().Alpha * 255.f)));
            window->DrawList->AddRect(ImVec2(window->Pos.x, window->Pos.y + 40),
                ImVec2(window->Pos.x + 128, window->Pos.y + window->Size.y - 40), ImColor(30, 30, 30, (int)(GetStyle().Alpha * 255.f)));

            window->DrawList->AddLine(ImVec2(window->Pos.x + 38, window->Pos.y + 83),
                ImVec2(window->Pos.x + 127, window->Pos.y + 83), ImColor(30, 30, 30, (int)(GetStyle().Alpha * 255.f)));

            window->DrawList->AddLine(ImVec2(window->Pos.x + 38, window->Pos.y + 123),
                ImVec2(window->Pos.x + 127, window->Pos.y + 123), ImColor(30, 30, 30, (int)(GetStyle().Alpha * 255.f)));

            window->DrawList->AddLine(ImVec2(window->Pos.x + 38, window->Pos.y + 163),
                ImVec2(window->Pos.x + 127, window->Pos.y + 163), ImColor(30, 30, 30, (int)(GetStyle().Alpha * 255.f)));
...
bool ImGui::MainBegin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
    IM_ASSERT(name != NULL && name[0] != '\0');     // Window name required
    ...
    if (first_begin_of_the_frame)
    {
        ...
        // FIXME: More code may rely on explicit sorting of overlapping child window and would need to disable this somehow. Please get in contact if you are affected.
        {
            ...
            const ImGuiWindow* window_to_highlight = g.NavWindowingTarget ? g.NavWindowingTarget : g.NavWindow;
            const bool title_bar_is_highlight = want_focus || (window_to_highlight && window->RootWindowForTitleBarHighlight == window_to_highlight->RootWindowForTitleBarHighlight);
            RenderMainWindowDecorations(window, title_bar_rect, title_bar_is_highlight, resize_grip_count, resize_grip_col, resize_grip_draw_size);
