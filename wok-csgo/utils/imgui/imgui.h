...
namespace ImGui
{
    ...
    IMGUI_API bool          MainBegin(const char* name, bool* p_open = NULL, ImGuiWindowFlags flags = 0);
    IMGUI_API void          End();
    ...
    IMGUI_API bool          subtab(const char* icon, const char* label, bool selected);
    ...
    IMGUI_API bool          ColorEdit4(const char* label, float col[4], bool keybind = false, ImGuiColorEditFlags flags = 0);
    IMGUI_API bool          ColorPicker3(const char* label, float col[3], ImGuiColorEditFlags flags = 0);
    IMGUI_API bool          ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags = 0, const float* ref_col = NULL);
    IMGUI_API bool          ColorButton(const char* desc_id, const ImVec4& col, bool keybind = false, ImGuiColorEditFlags flags = 0, ImVec2 size = ImVec2(0, 0)); // display a color square/button, hover for details, return true when pressed.
    ...
    IMGUI_API void          OpenPopupOnItemClick(const char* str_id = NULL, ImGuiPopupFlags popup_flags = 1);                     // helper to open popup when clicked on last item (note: actually triggers on the mouse _released_ event to be consistent with popup behaviors). return true when just opened.
...
struct ImGuiIO
{
    ...
    LPDIRECT3DTEXTURE9 Maintexture = nullptr;
...
struct ImDrawList
{
    ...
    IMGUI_API void  PathRect(const ImVec2& rect_min, const ImVec2& rect_max, float rounding = 0.0f, ImDrawFlags rounding_corners = ImDrawCornerFlags_All);
