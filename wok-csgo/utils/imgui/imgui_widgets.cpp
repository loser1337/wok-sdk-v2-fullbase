...
bool ImGui::tab(const char* icon, const char* label, bool selected)
{
    ...
    it_hover->second = ImClamp(it_hover->second + (1.15f * ImGui::GetIO().DeltaTime * (hovered ? 1.f : -1.f)), 0.0f, 0.2f);
    ...
    this_alpha->second = ImClamp(this_alpha->second + (2.15f * ImGui::GetIO().DeltaTime * (GetStyle().Alpha > 0.5f ? 1.f : -1.5f)), 0.3f, 1.f);
    this_alpha->second *= GetStyle().Alpha;

    auto ga = ImGui::GetStyle().Alpha;
    //149, 201, 56
    ImVec4 col2 = ImLerp(ImVec4{ 74 / 255.f, 74 / 255.f, 74 / 255.f, GetStyle().Alpha }, ImVec4{ 0 / 255.f, 172 / 255.f, 245 / 255.f, ImMin(this_alpha->second, 1.0f) }, it_filled->second);
    PushFont(fonts::icon_font);
    label_size = CalcTextSize(icon);
    window->DrawList->AddText(ImVec2(bb.Min.x + (20 - label_size.x / 2), bb.Min.y + (size.y / 2 - label_size.y / 2)), ImColor(col2), icon);
    PopFont();

    PushFont(fonts::font);
    label_size = CalcTextSize(label);
    window->DrawList->AddText(ImVec2(bb.Min.x + 40, bb.Min.y + (size.y / 2 - label_size.y / 2)), ImColor(col2), label);
    PopFont();

    return pressed;
}

bool ImGui::subtab(const char* icon, const char* label, bool selected)
{
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    it_hover->second = ImClamp(it_hover->second + (1.15f * ImGui::GetIO().DeltaTime * (hovered ? 1.f : -1.f)), 0.0f, 0.12f);
    it_hover->second *= GetStyle().Alpha;
    static std::map<ImGuiID, float> filled_animation;
    auto it_filled = filled_animation.find(id);
    if (it_filled == filled_animation.end())
    ...
    window->DrawList->AddText(ImVec2(bb.Min.x + 40, bb.Min.y + (size.y / 2 - label_size.y / 2)), ImColor(col2), label);
    PopFont();
    window->DrawList->AddRectFilled(ImVec2(bb.Max.x - 2, bb.Min.y + 10), ImVec2(bb.Max.x, bb.Max.y - 10), ImColor(0 / 255.f, 172 / 255.f, 245 / 255.f, it_filled->second), 6, ImDrawCornerFlags_Left);
...
bool ImGui::KeybindButton(const char* label, const char* unique_id, const ImVec2& size_arg, bool clicked, ImGuiButtonFlags flags)
{
    ...
    if (window->SkipItems)
        return false;
    SetCursorPosX(window->Size.x - 10 - size_arg.x);
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
    ...
    it_hover->second = ImClamp(it_hover->second + (3.f * ImGui::GetIO().DeltaTime * ((hovered || clicked || IsPopupOpen(unique_id)) ? 1.f : -1.f)), 0.0f, 1.f);
    ...
    it_open->second = ImClamp(it_open->second + (10.f * ImGui::GetIO().DeltaTime * ((clicked || IsPopupOpen(unique_id)) ? 1.f : -1.f)), 0.01f, 1.f);
    ...
    // Render
    ImVec4 textcolor = ImLerp(ImVec4(201 / 255.f, 204 / 255.f, 210 / 255.f, GetStyle().Alpha), ImVec4(255 / 255.f, 255 / 255.f, 255 / 255.f, GetStyle().Alpha), it_hover->second);

    ImGui::GetWindowDrawList()->AddRectFilled(bb.Min, bb.Max, ImColor(22, 22, 22, (int)(255 * g.Style.Alpha)), 4.f, IsPopupOpen(unique_id) ? ImDrawCornerFlags_Top : 0);
    ImGui::GetWindowDrawList()->AddRect(bb.Min, bb.Max, ImColor(30, 30, 30, (int)(g.Style.Alpha * 255)), 4.f, IsPopupOpen(unique_id) ? ImDrawCornerFlags_Top : 0);
    ...
    SetNextWindowSize(ImVec2(size_arg.x, CalcMaxPopupHeightFromItemCount(3) * it_open->second));
    if (ImGui::BeginPopup(unique_id, window_flags))
    {
        ImGui::PushStyleVar(ImGuiStyleVar_Alpha, it_open->second);
        static bool toggle = true, always, hold;
        SetCursorPosX(5);
        ImGui::BeginGroup();
        if (ImGui::Selectable("Enable", always))toggle = false, hold = false, always = true;
        if (ImGui::Selectable("Hold", hold))toggle = false, hold = true, always = false;
        if (ImGui::Selectable("Toggle", toggle))toggle = true, hold = false, always = false;
        ImGui::EndGroup();
        PopStyleVar();
        ImGui::EndPopup();
    }
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    if (*v)
        RenderCheckMark(window->DrawList, check_bb.Min + ImVec2(5, 5), ImColor(255, 255, 255, (int)(255 * it_filled->second)), 9);
...
bool ImGui::RadioButton(const char* label, bool active)
{
    ...
    const ImRect check_bb(pos + ImVec2(15, 0), pos + ImVec2(square_sz + 15, square_sz));
    const ImRect total_bb(pos + ImVec2(15, 0), pos + ImVec2(square_sz + 15 + (label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f), label_size.y + style.FramePadding.y * 2.0f));
    ItemSize(total_bb, style.FramePadding.y);
    if (!ItemAdd(total_bb, id))
        return false;
    ...
    window->DrawList->AddCircleFilled(center, 8, ImColor(22, 22, 22, (int)(g.Style.Alpha * 255)), 16);
    window->DrawList->AddCircle(center, 8, ImColor(30, 30, 30, (int)(g.Style.Alpha * 255)), 16);
    ...
    it_filled->second = ImClamp(it_filled->second + (2.35f * ImGui::GetIO().DeltaTime * (active ? 2.f : -2.f)), it_hover->second, 1.f);
    it_filled->second *= g.Style.Alpha;

    window->DrawList->AddCircleFilled(center, 8, ImColor(19, 148, 250, (int)(it_filled->second * 255)), 16);
    if (active)
        window->DrawList->AddCircleFilled(center, 3, ImColor(255, 255, 255, (int)(it_filled->second * 255)), 16);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    if (!(flags & ImGuiComboFlags_NoPreview)) {
        ...
        window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, ImColor(73, 73, 73, int(255 * GetStyle().Alpha)), 4);
    }
    ...
    {
        ImVec2 preview_pos = frame_bb.Min + ImVec2(style.FramePadding.x, (12 - CalcTextSize(preview_value).y / 2));
        if (g.LogEnabled)
            LogSetNextTextDecoration("{", "}");
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(15, 8), window->DC.CursorPos + ImVec2(window->Size.x - 15, label_size.y + 24));
    ...
    if (std::string(label).at(0) != '#' && std::string(label).at(1) != '#')
        window->DrawList->AddText(frame_bb.Min + ImVec2(0, 0), ImColor(73 / 255.f, 73 / 255.f, 73 / 255.f, g.Style.Alpha), label);

    ImRect slider_bb = ImRect(frame_bb.Min + ImVec2(0, label_size.y + 5), frame_bb.Max - ImVec2(0, 4));
    ...
    window->DrawList->AddRectFilled(ImVec2(slider_bb.Min.x, slider_bb.Min.y), ImVec2(slider_bb.Max.x, slider_bb.Max.y), ImColor(22, 22, 22, (int)(g.Style.Alpha * 255)), 6);
    window->DrawList->AddRect(ImVec2(slider_bb.Min.x, slider_bb.Min.y), ImVec2(slider_bb.Max.x, slider_bb.Max.y), ImColor(30, 30, 30, (int)(g.Style.Alpha * 255)), 6);
    window->DrawList->AddRectFilled(slider_bb.Min, ImVec2(slider_bb.Min.x + (g.Style.Alpha * it_old_value->second), slider_bb.Max.y), ImColor(19, 148, 250, (int)(255 * g.Style.Alpha)), 6, ImDrawCornerFlags_Left);
    window->DrawList->AddCircleFilled(ImVec2(slider_bb.Min.x + (g.Style.Alpha * it_old_value->second), slider_bb.Min.y + 3.5f), 6, ImColor(255, 255, 255, (int)(g.Style.Alpha * 255)));
    //window->DrawList->AddRectFilled(ImVec2(slider_bb.Min.x + (g.Style.Alpha * it_old_value->second) - 2, slider_bb.Min.y - 2), ImVec2(slider_bb.Min.x + 2 + (g.Style.Alpha * it_old_value->second), slider_bb.Max.y + 2), ImColor(255 / 255.f, 255 / 255.f, 255 / 255.f, GetStyle().Alpha), 6);

  //  window->DrawList->AddRectFilled(ImVec2(slider_bb.Max.x + 10, slider_bb.Min.y - 4), ImVec2(slider_bb.Max.x + 50, slider_bb.Max.y + 4), ImColor(36, 52, 74, (int)(g.Style.Alpha * 255)), 4);
   // window->DrawList->AddRect(ImVec2(slider_bb.Max.x + 10, slider_bb.Min.y - 4), ImVec2(slider_bb.Max.x + 50, slider_bb.Max.y + 4), ImColor(58, 79, 110, (int)(g.Style.Alpha * 255)), 4);

    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, (p_data), format);
    window->DrawList->AddText(ImVec2(frame_bb.Max.x - CalcTextSize(value_buf).x, frame_bb.Min.y), ImColor(205 / 255.f, 205 / 255.f, 205 / 255.f, g.Style.Alpha), value_buf);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(15, 0), window->DC.CursorPos + ImVec2(window->Size.x - 15, 24));
        ...
        if (io.InputQueueCharacters.Size > 0)
        {
            if (!ignore_char_inputs && !is_readonly && !user_nav_input_start)
                for (int n = 0; n < io.InputQueueCharacters.Size; n++)
                {
                    ...
                    if (c == '\t' && io.KeyShift)
                        continue;
                    if (InputTextFilterCharacter(&c, flags, callback, callback_user_data, ImGuiInputSource_NavKeyboard))
    ...
    ImVec2 draw_pos = is_multiline ? draw_window->DC.CursorPos : frame_bb.Min + ImVec2(style.FramePadding.x, 12 - CalcTextSize("S").y / 2);
...
{
    return ColorEdit4(label, col, false, flags | ImGuiColorEditFlags_NoAlpha);
}
...
bool ImGui::ColorEdit4(const char* label, float col[4], bool keybind, ImGuiColorEditFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    flags = (flags & (~ImGuiColorEditFlags__DisplayMask)) | ImGuiColorEditFlags_AlphaBar | ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_DisplayRGB | ImGuiColorEditFlags_NoOptions;
    ...
    const ImVec2 pos = window->DC.CursorPos;
    const float inputs_offset_x = 0.0f;
    ...
    it_open->second.mult = ImClamp(it_open->second.mult + 8.f * GetIO().DeltaTime * (it_open->second.play ? 1.f : -1.f), 0.01f, 1.f);
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (ColorButton("ColorButtons", col_v4, keybind, flags, ImVec2(20, 20)))
        {
            if (!(flags & ImGuiColorEditFlags_NoPicker))
            {
                ...
                SetNextWindowPos(window->DC.LastItemRect.GetBL() + ImVec2(-1, 0));
            }
        }
        if (!(flags & ImGuiColorEditFlags_NoOptions))
            OpenPopupOnItemClick("context");
        SetNextWindowSize(ImVec2(20 + 200 * it_open->second.mult, 187 * it_open->second.mult)); // Use 256 + bar sizes?zzzzz
        ImGuiWindowFlags window_flags = ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_Popup | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoMove;

        if (BeginPopup("picker", window_flags))
        {
            picker_active_window = g.CurrentWindow;
            //if (label != label_display_end)
            //{
            //    TextEx(label, label_display_end); //maximbulldozer
            //    Spacing();
            //}
            ...
            SetNextItemWidth(square_sz * 10.2f); // Use 256 + bar sizes?
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col) {
    ...
    float bars_width = square_sz / 1.6f; // Arbitrary smallish width of Hue/Alpha picking bars
    ...
    if (alpha_bar)
    {
        ...
        draw_list->AddRectFilledMultiColor(bar1_bb.Min, bar1_bb.Max, ImColor(255, 255, 255, (int)(g.Style.Alpha * 255)), ImColor(255, 255, 255, (int)(g.Style.Alpha * 255)), ImColor(0, 0, 0, (int)(g.Style.Alpha * 255)), ImColor(0, 0, 0, (int)(g.Style.Alpha * 255)));
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, bool keybind, ImGuiColorEditFlags flags, ImVec2 size)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    if (keybind)
        ImGui::SetCursorPosX(window->Size.x - 93);
    else
        ImGui::SetCursorPosX(window->Size.x - 28);
    ImGuiContext& g = *GImGui;
    ...
    ImVec4 col_rgb(col.x, col.y, col.z, col.w * GetStyle().Alpha);
    ...
    {
        RenderColorRectWithAlphaCheckerboard(window->DrawList, bb_inner.Min, bb_inner.Max, GetColorU32(col_rgb), grid_step, ImVec2(off, off), 26);
        window->DrawList->AddCircle(bb_inner.GetCenter(), size.x / 2 - 1, ImColor(255, 255, 255, int(255 * GetStyle().Alpha)));
        window->DrawList->AddCircleFilled(bb_inner.GetCenter(), size.x / 2 - 1, ImColor(col_rgb));
        for (int i = 1; i < 5; i++)
            window->DrawList->AddCircle(bb_inner.GetCenter(), size.x / 2 + i, ImColor(18, 18, 18, int(255 * GetStyle().Alpha)), 36, 4);
    }
    else
    {
        RenderColorRectWithAlphaCheckerboard(window->DrawList, bb_inner.Min, bb_inner.Max, GetColorU32(col_rgb), grid_step, ImVec2(off, off), 26);
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    if (pressed || (hovered && (flags & ImGuiSelectableFlags_SetNavIdOnHover)))
    {
        if (!g.NavDisableMouseHover && g.NavWindow == window && g.NavLayer == window->DC.NavLayerCurrent)
        {
            ...
            g.NavWindow->NavRectRel[window->DC.NavLayerCurrent] = ImRect(bb.Min - window->Pos, bb.Max - window->Pos);
            g.NavDisableHighlight = true;
        }
    }
    if (pressed)
    ...
    ImColor t_col = ((selected && hovered) ? ImColor(19, 148, 250, int(255 * g.Style.Alpha)) : hovered ? ImColor(177 / 255.f, 177 / 255.f, 177 / 255.f, 0.7f * g.Style.Alpha) : selected ? ImColor(19, 148, 250, int(255 * g.Style.Alpha)) : ImColor(170, 170, 170, int(170 * g.Style.Alpha)));
